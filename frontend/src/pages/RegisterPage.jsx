...
export default function RegisterPage() {
  ...
  return (
    <div className="mx-auto max-w-md glass p-6">
      ...
      <p className="mt-2 text-slate-400">Create your account to start shopping.</p>
      <form className="mt-6 space-y-4" onSubmit={onSubmit}>
        <div>
          <input className="input-field" type="text" placeholder="Full name" value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} />
      ...
      <p className="mt-5 text-sm text-slate-400">
        Already registered? <Link to="/login" className="text-blue-300">Login</Link>
      </p>
    </div>
  );
}
